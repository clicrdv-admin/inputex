/**
 * Source http://arantius.com/inc/tabifier.js
 * Bug:
 *  <script></script> will render as <script><</script>
 */
...
function runTabifier(inputEl, outputEl, type) {
    tabifierOutput = outputEl
	var code=document.getElementById(inputEl).value;
...
function finishTabifier(code) {
	...
	document.getElementById(tabifierOutput).value=code;
	level=0;
	...
	function cleanAsync() {
		...
		for (; i<code.length && i<iStart+LOOP_SIZE; i++) {
			...
			if ('!--'==tag.substr(1,3)) {
				...
				i+=end;
				out+=cont+'<';
				//if ('style'==t[1]) out+=cleanCSS(cont);
				//if ('script'==t[1]) out+='\n'+cleanCStyle(cont)+'\n'+tabs();
			} else {
				tag=cleanTag(tag);
				out=placeTag(tag, out);
...
function tabs() {
	var s='';
	for (var j=0; j<level; j++) s+='    ';//changed \t to '    '
	return s;
}
